        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for value in ['01.2.3\n', '1.2\n', '1.2.3', '1.2.3\n\n', '1.2.3+local\n']:
                ...
                with self.assertRaises(ValueError):
                    contract.version(root)

    def test_selection(self):
        self.assertFalse(contract.full_selected('pull_request', {}, ['README.md', 'docs/guide.md']))
        for event in ['push', 'merge_group', 'workflow_dispatch']:
            self.assertTrue(contract.full_selected(event, {}, ['README.md']))
        for paths in [None, [], ['flake.lock'], ['packages/new.nix'], ['unknown/script.py'], ['docs/example.nix']]:
            self.assertTrue(contract.full_selected('pull_request', {}, paths))
        self.assertTrue(contract.full_selected('pull_request', {'pull_request': {'labels': [{'name': 'nix-ci'}]}}, ['README.md']))
